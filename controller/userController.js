...
const userController = {
    ...
    updateAvatar: async (req, res) => {
        try {
            ...
            const updateAvatarUser = await User.findByIdAndUpdate(
                ...
                { new: true } /*trả về tài liêu đã dc câp nhât thay vì tài liêu cũ*/
            );
    ...
    sendResetPasswordEmail: async (req, res) => {
        ...
        try {
            ...
            await transporter.sendMail({
                ...
                html: `
                <p>Vui lòng nhập mã xác thực dưới đây để đặt lại mật khẩu:</p>
                <h2 style="font-weight: bold;">${verificationCode}</h2>
                <p>Mã xác thực sẽ hết hạn sau 15 phút.</p>
            `,
            });

            user.otp = verificationCode;
            user.otpExpiresAt = now + OTP_EXPIRATION_TIME;
            user.otpRequestedAt = now;

            await user.save();

            return res.status(200).json({ message: 'Mã xác thực đã được gửi đến email' });
        } catch (error) {
    ...
    verifyOtp: async (req, res) => {
        ...
        try {
            // Tìm người dùng có OTP trong cơ sở dữ liệu
            const user = await User.findOne({ otp });

            if (!user) return res.status(404).json({ message: 'Mã OTP không chính xác' });

            if (Date.now() > user.otpExpiresAt) return res.status(400).json({ message: 'Mã OTP đã hết hạn' });
    ...
    resetPassword: async (req, res) => {
        ...
        try {
            ...
            console.error('Cập mật khẩu mới lỗi:', error);
